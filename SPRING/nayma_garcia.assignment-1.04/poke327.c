...
# define randRange(min, max) ((rand() % (((max) + 1) - (min))) + (min)) //makes a random range for whatver trainer to move in
...
typedef struct world {
  ...
  int trainer[MAP_Y][MAP_X];
  int hiker_dist[MAP_Y][MAP_X];
  int rival_dist[MAP_Y][MAP_X];
  int pacer_dist[MAP_Y][MAP_X];
  int wanderer_dist[MAP_Y][MAP_X];
  int sentries_dist[MAP_Y][MAP_X];
  int explorer_dist[MAP_Y][MAP_X];
...
static int32_t pacer_cmp(const void *key, const void *with) {
  return (world.pacer_dist[((path_t *) key)->pos[dim_y]]
                          [((path_t *) key)->pos[dim_x]] -
          world.pacer_dist[((path_t *) with)->pos[dim_y]]
                          [((path_t *) with)->pos[dim_x]]);
}

static int32_t wanderer_cmp(const void *key, const void *with) {
  return (world.wanderer_dist[((path_t *) key)->pos[dim_y]]
                          [((path_t *) key)->pos[dim_x]] -
          world.wanderer_dist[((path_t *) with)->pos[dim_y]]
                          [((path_t *) with)->pos[dim_x]]);
}

static int32_t sentries_cmp(const void *key, const void *with) {
  return (world.sentries_dist[((path_t *) key)->pos[dim_y]]
                          [((path_t *) key)->pos[dim_x]] -
          world.sentries_dist[((path_t *) with)->pos[dim_y]]
                          [((path_t *) with)->pos[dim_x]]);
}

static int32_t explorer_cmp(const void *key, const void *with) {
  return (world.explorer_dist[((path_t *) key)->pos[dim_y]]
                          [((path_t *) key)->pos[dim_x]] -
          world.explorer_dist[((path_t *) with)->pos[dim_y]]
                          [((path_t *) with)->pos[dim_x]]);
}
...
//HIkeR-----------------------------------------------------------------------------
  ...
  //pacer-------------------------------------------------------------
   ...
   heap_init(&h, pacer_cmp, NULL);

  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      if (ter_cost(x, y, char_pacer) != INT_MAX) {
        p[y][x].hn = heap_insert(&h, &p[y][x]);
      } else {
        p[y][x].hn = NULL;
      }
  ...
  while ((c = heap_remove_min(&h))) {
    c->hn = NULL;
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x] - 1].hn) &&
        (world.pacer_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] >
         world.pacer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_pacer))) {
      world.pacer_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] =
        world.pacer_dist[c->pos[dim_y]][c->pos[dim_x]] +
        ter_cost(c->pos[dim_x], c->pos[dim_y], char_pacer);
      heap_decrease_key_no_replace(&h,
    ...
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x]    ].hn) &&
        (world.pacer_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] >
         world.pacer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_pacer))) {
      world.pacer_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] =
        world.pacer_dist[c->pos[dim_y]][c->pos[dim_x]] +
    ...
    if ((p[c->pos[dim_y]    ][c->pos[dim_x] - 1].hn) &&
        (world.pacer_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] >
         world.pacer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_pacer))) {
      world.pacer_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] =
        world.pacer_dist[c->pos[dim_y]][c->pos[dim_x]] +
 ...
 // WANDERER
  ...
  heap_init(&h, wanderer_cmp, NULL);

  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      if (ter_cost(x, y, char_wanderer) != INT_MAX) {
        p[y][x].hn = heap_insert(&h, &p[y][x]);
      } else {
        p[y][x].hn = NULL;
      }
  ...
  while ((c = heap_remove_min(&h))) {
    c->hn = NULL;
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x] - 1].hn) &&
        (world.wanderer_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] >
         world.wanderer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_wanderer))) {
      world.wanderer_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] =
        world.wanderer_dist[c->pos[dim_y]][c->pos[dim_x]] +
        ter_cost(c->pos[dim_x], c->pos[dim_y], char_wanderer);
      heap_decrease_key_no_replace(&h,
    ...
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x]    ].hn) &&
        (world.wanderer_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] >
         world.wanderer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_wanderer))) {
      world.wanderer_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] =
        world.wanderer_dist[c->pos[dim_y]][c->pos[dim_x]] +
    ...
    if ((p[c->pos[dim_y]    ][c->pos[dim_x] - 1].hn) &&
        (world.wanderer_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] >
         world.wanderer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_wanderer))) {
      world.wanderer_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] =
        world.wanderer_dist[c->pos[dim_y]][c->pos[dim_x]] +
...
//sentries
   ...
   heap_init(&h, sentries_cmp, NULL);

  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      if (ter_cost(x, y, char_sentries) != INT_MAX) {
        p[y][x].hn = heap_insert(&h, &p[y][x]);
      } else {
        p[y][x].hn = NULL;
      }
  ...
  while ((c = heap_remove_min(&h))) {
    c->hn = NULL;
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x] - 1].hn) &&
        (world.sentries_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] >
         world.sentries_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_sentries))) {
      world.sentries_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] =
        world.sentries_dist[c->pos[dim_y]][c->pos[dim_x]] +
        ter_cost(c->pos[dim_x], c->pos[dim_y], char_sentries);
      heap_decrease_key_no_replace(&h,
    ...
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x]    ].hn) &&
        (world.sentries_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] >
         world.sentries_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_sentries))) {
      world.sentries_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] =
        world.sentries_dist[c->pos[dim_y]][c->pos[dim_x]] +
    ...
    if ((p[c->pos[dim_y]    ][c->pos[dim_x] - 1].hn) &&
        (world.sentries_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] >
         world.sentries_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_sentries))) {
      world.sentries_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] =
        world.sentries_dist[c->pos[dim_y]][c->pos[dim_x]] +
...
//explorer
   ...
   heap_init(&h, explorer_cmp, NULL);

  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      if (ter_cost(x, y, char_explorer) != INT_MAX) {
        p[y][x].hn = heap_insert(&h, &p[y][x]);
      } else {
        p[y][x].hn = NULL;
      }
  ...
  while ((c = heap_remove_min(&h))) {
    c->hn = NULL;
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x] - 1].hn) &&
        (world.explorer_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] >
         world.explorer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_explorer))) {
      world.explorer_dist[c->pos[dim_y] - 1][c->pos[dim_x] - 1] =
        world.explorer_dist[c->pos[dim_y]][c->pos[dim_x]] +
        ter_cost(c->pos[dim_x], c->pos[dim_y], char_explorer);
      heap_decrease_key_no_replace(&h,
    ...
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x]    ].hn) &&
        (world.explorer_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] >
         world.explorer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_explorer))) {
      world.explorer_dist[c->pos[dim_y] - 1][c->pos[dim_x]    ] =
        world.explorer_dist[c->pos[dim_y]][c->pos[dim_x]] +
    ...
    if ((p[c->pos[dim_y]    ][c->pos[dim_x] - 1].hn) &&
        (world.explorer_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] >
         world.explorer_dist[c->pos[dim_y]][c->pos[dim_x]] +
         ter_cost(c->pos[dim_x], c->pos[dim_y], char_explorer))) {
      world.explorer_dist[c->pos[dim_y]    ][c->pos[dim_x] - 1] =
        world.explorer_dist[c->pos[dim_y]][c->pos[dim_x]] +
...
//function to move the trainers in a line to their new position
  void moveTrainer(trainer_t *trainer, int playerY, int playerX){
      if(trainer->pos[1] < playerX){
	trainer->next_pos[0] = trainer->pos[0];
	trainer->next_pos[1] = trainer->pos[1] + 1;
      }
      else if(trainer->pos[1] > playerX){
	trainer->next_pos[0] = trainer->pos[0];
	trainer->next_pos[1] = trainer->pos[1] - 1;
      }
      else if(trainer->pos[0] < playerY){
	trainer->next_pos[0] = trainer->pos[0] + 1;
	trainer->next_pos[1] = trainer->pos[1];
      }
      else if(trainer->pos[0] > playerY){
	trainer->next_pos[0] = trainer->pos[0] - 1;
	trainer->next_pos[1] = trainer->pos[1];
...
//function to decide how they move
void generate_move(world_t *world, heap_t *heap, trainer_t *trainer, int playerY, int playerX){
  int maxGradient = rand() % 2;
  int cantTraverse = rand() % 3;
  
    
  if(trainer->type == '@'){
    
    int nextY = randRange(-1, 1);
    int nextX = randRange(-1, 1);
    trainer->next_pos[0] = trainer->pos[0] + nextY;
    trainer->next_pos[1] = trainer->pos[1] + nextX;
  }else if(trainer->type == char_hiker){
    ...
    else if(cantTraverse){
      moveTrainer(trainer, playerY, playerX);
    }
    else{
      trainer->next_pos[0] = trainer->pos[0];
      trainer->next_pos[1] = trainer->pos[1];
  ...
  else if(trainer->type == char_rival){
  ...
  else if(trainer->type == char_pacer){
  ...
  else if(trainer->type == char_wanderer){
    
    moveTrainer(trainer, playerY, playerX);
  }
  else if(trainer->type == '5'){
  ...
  else if(trainer->type == char_sentries){
  
    moveTrainer(trainer, playerY, playerX);
  }
  else if(trainer->type =='7'){
  ...
  else if(trainer->type == char_explorer){
  
    if(cantTraverse){
      
      trainer->lastPos[0] = playerY;
      trainer->lastPos[1] = playerX;
    }
    else if(trainer->lastPos[0] != 0 && trainer->lastPos[1] != 0){
      moveTrainer(trainer, trainer->lastPos[0], trainer->lastPos[1]);
      trainer->lastPos[0] = 0;
      trainer->lastPos[1] = 0;
    }
    else{
  ...
  else if(trainer->type == char_swimmer){
  ...
  if((trainer->type == char_pc || trainer->type == char_hiker || trainer->type == char_rival || trainer->type == char_pacer || trainer->type == char_wanderer || trainer->type == char_sentries || trainer->type == char_explorer || trainer->type == char_swimmer) &&
     world->world[trainer->next_pos[0]][trainer->next_pos[1]] != 0){
    trainer->next_pos[0] = trainer->pos[0];
    trainer->next_pos[1] = trainer->pos[1];
    trainer->turn = trainer->turn + 1000 / trainer->movementType;
    trainer->hn = heap_insert(heap, trainer);
  }
...
//making the switch for the number of trainers
void usage(const char *argv0) {

  fprintf(stderr,
          "Usage: %s [--numtrainers|--seed] <int>\n",argv0);
  exit(-1);
}
