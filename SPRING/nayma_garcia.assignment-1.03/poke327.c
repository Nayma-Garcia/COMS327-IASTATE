...
static void dijkstra_pathCost(map_t *m, pair_t from, pair_t to, pair_t rival, pair_t hiker){
  static path_t path[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
...
void print_dist_map(int32_t dist_map[][MAP_X], map_t *m) {
pair_t from;
pair_t to;
pair_t rival;
pair_t hiker;
  dijkstra_pathCost(m, from, to, rival, hiker);
  for (int32_t i = 0; i < MAP_Y; i++) {
    for (int32_t j = 0; j < MAP_X; j++) {
...
static int map_terrain(map_t *m, int8_t n, int8_t s, int8_t e, int8_t w)
{
  ...
  queue_node_t *head, *tail, *tmp;
  //  FILE *out;
  int num_grass, num_clearing, num_mountain, num_forest, num_water, num_total, num_pc;
  terrain_type_t type;
  int added_current = 0;
  ...
  num_water = rand() % 2 + 1;
  num_pc = rand() % 2 + 1;
  num_total = num_grass + num_clearing + num_mountain + num_forest + num_water + num_pc;
